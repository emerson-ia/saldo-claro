...
O app agora possui tela de entrada, cadastro por e-mail/senha, recuperação de senha, saída e fluxo preparado para Google. A sessão é persistida com segurança no aparelho e os dados locais passam a usar uma chave separada por usuário.

Para ativar a autenticação real:

1. Crie um projeto gratuito no Supabase.
2. Em **Authentication > Providers**, habilite **Email**. Para Google, habilite o provedor Google e informe o Client ID e Client Secret no Supabase.
3. Em **Authentication > URL Configuration**, adicione `saldo-claro://auth/callback` como Redirect URL. Para o Expo Go durante desenvolvimento, adicione também o endereço `exp://.../--/auth/callback` que o Expo mostrar no terminal.
4. Copie `.env.example` para `.env.local` e preencha `EXPO_PUBLIC_SUPABASE_URL` e `EXPO_PUBLIC_SUPABASE_ANON_KEY`.
5. Reinicie o Expo após criar o `.env.local`.

A chave anônima do Supabase pode ficar no app. Nunca adicione `service_role`, Client Secret do Google ou qualquer senha ao repositório.
...
O Finanças em Dia começa no plano **Gratuita**, com todas as funcionalidades atuais liberadas para sempre. A tela **Perfil → Versão Gratuita** deixa essa regra explícita e apresenta o futuro plano **PRO**, reservado para automações e integrações com bancos e instituições financeiras.

Não há cobrança, checkout ou coleta de cartão ativados nesta etapa. A migração `drizzle/0001_subscription_foundation.sql` já prepara as tabelas `subscriptions` e `billing_webhook_events` para a ativação futura com qualquer provedor de pagamentos. O desenho guarda o plano e os IDs externos, permite cancelamento no fim do ciclo e registra webhooks de forma idempotente. Segredos de pagamento e o endpoint de webhook só devem ser configurados quando a cobrança for aprovada para lançamento.
...
O nome, textos, paleta e ícone do Finanças em Dia são originais. A cor de marca é o verde-petróleo `#0B6B62`, com verde para resultados positivos, âmbar para atenção e vermelho para despesas ou risco. A configuração central de marca fica em `app.config.ts`.
