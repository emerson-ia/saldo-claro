...
export function AppHeader({ title, subtitle, back = false, onBack, right }: { title: string; subtitle?: string; back?: boolean; onBack?: () => void; right?: ReactNode }) {
  ...
  return <View style={styles.header}>
    <View style={styles.headerTitleRow}>
      {back ? <Pressable accessibilityLabel="Voltar" onPress={() => onBack ? onBack() : router.canGoBack() ? router.back() : router.replace('/')} style={({ pressed }) => [styles.back, { backgroundColor: colors.elevated, opacity: pressed ? 0.7 : 1 }]}><MaterialIcons name="arrow-back" size={21} color={colors.text} /></Pressable> : null}
...
export function IconButton({ icon, label, onPress, tone = "neutral" }: { icon: keyof typeof MaterialIcons.glyphMap; label: string; onPress: () => void; tone?: "neutral" | "primary" }) {
  ...
  const color = tone === "primary" ? colors.onPrimary : colors.text;
