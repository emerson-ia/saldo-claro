...
export default function MoreScreen() {
  ...
  return <ScreenContainer><FlatList data={menu} keyExtractor={(item) => item.label} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}
    ...
    ListFooterComponent={<View style={{ marginTop: 19 }}><Text style={[styles.section, { color: colors.muted }]}>FERRAMENTAS</Text><Pressable onPress={() => router.push("/import-csv" as never)} style={({ pressed }) => [styles.tool, { backgroundColor: colors.surface, opacity: pressed ? .7 : 1 }]}><View style={[styles.menuIcon, { backgroundColor: "#FFF2D8" }]}><MaterialIcons name="file-upload" size={20} color={colors.warning} /></View><View style={styles.menuCopy}><Text style={[styles.menuTitle, { color: colors.text }]}>Importar CSV ou OFX</Text><Text style={[styles.menuDescription, { color: colors.muted }]}>Adicione lançamentos da planilha ou extrato bancário.</Text></View><MaterialIcons name="chevron-right" size={22} color={colors.muted} /></Pressable><Text style={[styles.version, { color: colors.muted }]}>Finanças em Dia · Versão de demonstração</Text></View>}
  /></ScreenContainer>;
}
